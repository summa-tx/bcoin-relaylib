...
class RelayIndexer extends Indexer {
  ...
  _deduplicateSatisfied(records) {
    // TODO: Better way to do this? Is this way reliable?
    return records.filter((e) => {
      return records.findIndex(f => f.id === e.id) === records.indexOf(e);
    });
  }
  /**
    ...
    for (let i = 0; i < block.txs.length; i++) {
      ...
      for (const [j, input] of Object.entries(tx.inputs)) {
        ...
        if (has(prevout.toRaw())) {
          ...
          this.logger.info('Filter hit: outpoint %s/%s', txidHex, index);
          ...
          satisfied = satisfied.concat(orecord.requests);
        }
      }
      ...
      for (const [j, output] of Object.entries(tx.outputs)) {
        ...
        if (has(script.toRaw())) {
          ...
          satisfied = satisfied.concat(srecord.requests);
        }
      }

      if (satisfied.length > 0) {
        this._deduplicateSatisfied(satisfied);

        // TODO: Deduplicate satisfied
