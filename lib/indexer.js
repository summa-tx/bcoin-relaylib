...
class RelayIndexer extends Indexer {
    ...
    for (let i = 0; i < block.txs.length; i++) {
      ...
      for (const [j, output] of Object.entries(tx.outputs)) {
        ...
        if (this.has(script.toRaw())) {
          ...
          const orecord = OutpointRecord.fromOptions({
            prevout: {
              hash: txid,
              index: index
            },
          ...
          await this.putOutpointRecord(orecord);
  ...
  async addRequest(request) {
    ...
    if (!request.pays.raw.equals(Buffer.alloc(0))) {
      ...
      await this.putScriptRecord(srecord);
    }
  ...
  async putScriptRecord(srecord, request) {
    ...
    if (await this.hasScriptRecord(srecord)) {
      ...
      const r = await this.getScriptRecord(srecord);
      r.add(request.id);
      srecord = r;
    }
